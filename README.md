# Hi, I'm Matt!

- 💼 Work as a software tester doing automation framework development
- 🎓 Pursing a BS in Computer Science at Oregon State University
- 📚 Currently studying database design and systems programming
- ⚔️ Current side quest: learning [Rust](https://doc.rust-lang.org/book/)! 🦀

<!-- [![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=mattmuroya&layout=compact&size_weight=0.5&count_weight=0.5&langs_count=10&hide=python,html,css,scss,cmake,gherkin,procfile,glsl&exclude_repo=procedural-terrain-generator)](https://github.com/anuraghazra/github-readme-stats) -->

<!--
<div align="center">
  <p>Aloha from Honolulu, HI 🌈</p>
</div>
-->
<!--
<div align="center">
  <img  src="https://streak-stats.demolab.com/?user=mattmuroya&theme=gotham&hide_border=true&background=00000000&dates=2AA789" />
  
  [![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=mattmuroya&layout=compact&size_weight=0.5&count_weight=0.5&langs_count=10&hide=html,css,scss,cmake,gherkin,procfile&exclude_repo=procedural-terrain-generator)](https://github.com/anuraghazra/github-readme-stats)



  <h2>Here's what I've been working on!</h2>

  <table>
    <tr valign="top">
      <td width="50%">
        <a href="https://poet-instant-messenger.herokuapp.com/">
          <img border="1px" src="assets/poet.png">
        </a>
        <h3>Poet Instant Messenger</h3>
        <p>Full stack React/Node.js chat application in the spirit of the classic IM apps of the late '90s.</p>
        <p>
          (Note: demo offline while I explore Heroku alternatives)
        </p>
        <p>
          <a href="https://github.com/mattmuroya/poet">GitHub</a>
        </p>
      </td>
      <td width="50%">
        <a href="https://www.mattmuroya.com/">
          <img border="1px" src="assets/mattmuroya.com.png">
        </a>
        <h3>mattmuroya.com</h3>
        <p>Routes and content statically generated from Markdown files using React/Next.js.</p>
        <p>
          <a href="https://github.com/mattmuroya/mattmuroya.com">GitHub</a> | <a href="https://www.mattmuroya.com/">Demo</a>
        </p>
      </td>
    </tr>
    <tr valign="top">
      <td width="50%">
        <a href="https://hawaii-state-parks-api.herokuapp.com/">
          <img border="1px" src="assets/hiapi.png">
        </a>
        <h3>Hawaii State Parks API</h3>
        <p>Python/Flask REST API providing descriptions and park data for each of Hawaii's state parks.</p>
        <p>
          (Note: demo offline while I explore Heroku alternatives)
        </p>
        <p>
          <a href="https://github.com/mattmuroya/hawaii-state-parks-api">GitHub</a>
        </p>
      </td>
      <td width="50%">
        <a href="https://checkmark.mattmuroya.com/">
          <img border="1px" src="assets/checkmark.png">
        </a>
        <h3>Checkmark</h3>
        <p>Fully responsive vanilla JavaScript task manager demonstrating basic OOP design principles.</p>
        <p>
          <a href="https://github.com/mattmuroya/checkmark">GitHub</a> | <a href="https://checkmark.mattmuroya.com/">Demo</a>
        </p>
      </td>
    </tr>
    <tr valign="top">
      <td width="50%">
        <a href="https://mattmuroya.github.io/the-lens-shop/">
          <img border="1px" src="assets/lens.png">
        </a>
        <h3>The Lens Shop</h3>
        <p>Mock storefront/shopping cart demonstrating state management with React.</p>
        <p>
          <a href="https://github.com/mattmuroya/the-lens-shop">GitHub</a> | <a href="https://mattmuroya.github.io/the-lens-shop/">Demo</a>
        </p>
      </td>
      <td width="50%">
        <a href="https://github.com/mattmuroya/go.moku">
          <img border="1px" src="assets/go.moku.png">
        </a>
        <h3>go.moku</h3>
        <p>Gomoku, but it's a command line game written in Go.</p>
        <p>
          <a href="https://github.com/mattmuroya/go.moku">GitHub</a>
        </p>
      </td>
    </tr>
  </table>  
</div>-->
